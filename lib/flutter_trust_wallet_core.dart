...
class FlutterTrustWalletCore {
  /// `libTrustWalletCore.dylib` will not be imported into the application
  /// through the plugin. It must be manually placed into the `lib/cli`
  /// directory of the project that uses the plugin.
  static void init({bool useCliLib = false}) {
    if (useCliLib) {
      ...
      walletCoreLib = DynamicLibrary.open(libraryPath);
      return;
    }
